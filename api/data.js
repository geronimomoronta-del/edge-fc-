export default async function handler(req, res) {
  ...
  try {
    ...
    // Probabilidades de API-Football
    const percent = prediction.predictions?.percent || {};

    const modelProb = {
      home: parseFloat(percent.home) / 100,
      draw: parseFloat(percent.draw) / 100,
      away: parseFloat(percent.away) / 100
    };

    const calculate = (name, odd, model, market) => {
      const fairOdd = model > 0 ? 1 / model : null;
      ...
      return {
        selection: name,
        odd,
        market_probability: +(market * 100).toFixed(2),
        model_probability: +(model * 100).toFixed(2),
        fair_odd: fairOdd ? +fairOdd.toFixed(2) : null,
    ...
    return res.status(500).json({
      ok: false,
      error: "No se pudo calcular el Value Finder"
    });
  }
}
  let url = "";
